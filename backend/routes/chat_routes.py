...
from utils.vectors import get_answer, get_streaming_answer
...
def get_user_chats(commons, user_id):
    response = commons['supabase'].from_('chats').select(
        'chatId:chat_id, chatName:chat_name').filter("user_id", "eq", user_id).execute()
    return response.data


def get_chat_details(commons, chat_id):
    response = commons['supabase'].from_('chats').select(
        '*').filter("chat_id", "eq", chat_id).execute()
    return response.data
...
async def get_chats_endpoint(commons: CommonsDep, current_user: User = Depends(get_current_user)):
    """
...
def chat_handler(request, commons, chat_id, chat_message, email, is_new_chat=False, streaming=False):
    """
...
async def update_chat_endpoint(request: Request, commons: CommonsDep, chat_id: UUID, chat_message: ChatMessage, current_user: User = Depends(get_current_user)):
    """
...
async def create_chat_endpoint(request: Request, commons: CommonsDep, chat_message: ChatMessage, current_user: User = Depends(get_current_user)):
    """
...
async def chat_streaming_handler(request, commons, chat_id, chat_message, email):
    """
    Helper method for update and create chat with streaming response.
    """
    date = time.strftime("%Y%m%d")
    user_id = fetch_user_id_from_credentials(commons, {"email": email})
    max_requests_number = os.getenv("MAX_REQUESTS_NUMBER")
    ...
    async for item in get_streaming_answer(
            commons, chat_message, email, user_openai_api_key):
        yield item


@chat_router.put("/chat/{chat_id}/stream", dependencies=[Depends(AuthBearer())], tags=["Chat"])
async def update_chat_stream_endpoint(request: Request, commons: CommonsDep, chat_id: UUID, chat_message: ChatMessage, current_user: User = Depends(get_current_user)):
    """
    Update an existing chat with new chat messages. This endpoint returns a Streaming Response.
    """

    return StreamingResponse(chat_streaming_handler(
        request, commons, chat_id, chat_message, current_user.email), media_type="text/event-stream")
