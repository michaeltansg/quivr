...
async def send_message(commons, chat_message: ChatMessage, current_user: User) -> AsyncIterable[str]:
    ...
    streaming_llm = ChatOpenAI(
        temperature=0, model_name=chat_message.model, streaming=True, callbacks=[callback])
    ...
    qa = ConversationalRetrievalChain(
        retriever=vector_store.as_retriever(), question_generator=question_generator,
    ...
    chat_history = []
    task = asyncio.create_task(wrap_done(await
                                         qa.a({"question": chat_message.question, "chat_history": chat_history}), callback.done))
...
def stream(commons: CommonsDep, chat_message: ChatMessage, current_user: User = Depends(get_current_user)):
    return StreamingResponse(send_message(commons, chat_message, current_user), media_type="text/event-stream")
